...
describe('Testes para a página de candidatura', () => {
    ...
    it('Deve preencher o formulário de inscrição', () => {
        cy.get('.Vaga_vagaLink__DeFkk').first().click()
        cy.get('input[name="nome-completo"]').type('manoela coelho')
        cy.get('input[name="email"]').type('manoela@teste.com')
        cy.get('input[name="telefone"]').type('11 12345678')
        cy.get('input[name="endereco"]').type('rua vscode, bairro cypress, florianópolis-sc')
        cy.get('#linux').check()
